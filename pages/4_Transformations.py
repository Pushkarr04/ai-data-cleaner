...
from modules.transform import show_transformation

st.set_page_config(page_title="Transformations | DataCleaner AI", page_icon="⚙️", layout="wide")
...
if st.session_state.get('cleaned_df') is not None:
    transformed, logs = show_transformation(st.session_state['cleaned_df'])
    if transformed is not None and logs:
        st.session_state['cleaned_df'] = transformed
        for log in logs:
            if log not in st.session_state.get('cleaning_logs', []):
                st.session_state.setdefault('cleaning_logs', []).append(log)
else:
    st.markdown("""
