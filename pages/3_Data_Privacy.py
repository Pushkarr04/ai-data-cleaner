...
from modules.privacy import show_privacy_options

st.set_page_config(page_title="Data Privacy | DataCleaner AI", page_icon="🛡️", layout="wide")
...
if st.session_state.get('cleaned_df') is not None:
    secured_df, logs = show_privacy_options(st.session_state['cleaned_df'])
    if secured_df is not None and logs:
        st.session_state['cleaned_df'] = secured_df
        for log in logs:
